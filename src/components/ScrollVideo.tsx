            ...
            case 'video':
                return (
                    <div style={{ marginTop: '30px', position: 'relative', width: isMobile ? '80%' : '500px', aspectRatio: '16/9', background: 'rgba(255,255,255,0.05)', borderRadius: '20px', border: '1px solid rgba(255,255,255,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
                        ...
                        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', padding: isMobile ? '0 20px 15px' : '0 40px 20px', textAlign: 'center' }}>
                            <div style={{ color: '#fff', fontWeight: 600, fontSize: isMobile ? '1.1rem' : '1.3rem', textShadow: '0 2px 10px rgba(0,0,0,0.8)' }}>{items[0].title}</div>
                            {items[0].description && <div style={{ color: 'rgba(255,255,255,0.6)', fontSize: '0.8rem', marginTop: '8px' }}>{items[0].description}</div>}
                        </div>
                    </div>
                );
