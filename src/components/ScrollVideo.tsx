...
export const ScrollVideo: React.FC = () => {
  ...
  // Throttle state updates from onUpdate to ~20fps (50ms) while
  // keeping frame drawing at full speed. This eliminates ~66% of
  // React re-renders from the scroll callback.
  const lastStateUpdateRef = useRef<number>(0);
  const pendingStateUpdateRef = useRef<number>(0);
  ...
  useLayoutEffect(() => {
    const m = window.matchMedia(MOBILE_QUERY).matches;
    setIsMobile(m);
  ...
  const { images, fallbackImage, progress, isLoaded, preloadFrames } =
    usePreload();
  ...
  useEffect(() => {
    ...
    const scrollTrigger = ScrollTrigger.create({
      ...
      onUpdate: (self) => {
        ...
        // Frame drawing runs at full speed — no throttle.
        ...
        // Frame drawing above stays at 60fps; only overlay UI updates
        // (service text, opacity, timeline) are throttled to avoid
        // excessive re-renders of the large JSX tree.
        ...
        const applyStateUpdate = () => {
          ...
          if (scrollProgress < textFadeStart) {
            ...
            setIntroTextOpacity(
              1 -
                (scrollProgress - textFadeStart) /
                  (textFadeEnd - textFadeStart),
            );
          } else {
          ...
          if (scrollProgress < introHoldEnd) {
            ...
            setIntroOpacity(
              1 -
                (scrollProgress - introHoldEnd) / (introFadeEnd - introHoldEnd),
            );
          } else {
            setIntroOpacity(0);
          }
        ...
        if (now - lastStateUpdateRef.current >= STATE_THROTTLE_MS) {
          ...
          applyStateUpdate();
        } else if (!pendingStateUpdateRef.current) {
          // Schedule a trailing update so we never miss the final position
          pendingStateUpdateRef.current = requestAnimationFrame(() => {
            pendingStateUpdateRef.current = 0;
